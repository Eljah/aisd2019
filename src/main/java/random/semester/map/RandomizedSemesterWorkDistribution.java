...
public class RandomizedSemesterWorkDistribution {
    public static void main(String[] args) {
        ...
        Map<String, Student> distribution = new TreeMap<>();
        ...
        while (studentsIterator.hasNext()) {
            ...
            System.out.println("Случайно определенный номер: " + randomNum);
            if (distribution.get(semesterWork.get(randomNum)) != null) {
                System.out.println("Это уже раздали");
            } else {
                Student current = studentsIterator.next();
                System.out.println("Выдаем семестровую работу (" + randomNum + ") " + semesterWork.get(randomNum) + " студенту " + current);
                distribution.put(semesterWork.get(randomNum), current);
            }
        }
        ;
        System.out.println();
        ...
        System.out.println();
        for (Map.Entry<String, Student> entry : distribution.entrySet()) {
            System.out.println(entry.getValue().getSecondName() + " " + entry.getValue().getFirstName() + ": " + entry.getKey().toString());
        }
    }
}
